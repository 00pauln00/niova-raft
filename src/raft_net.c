...
raft_net_instance_shutdown(struct raft_instance *ri)
{
    ...
    if (ri->ri_init_cb)
        ri->ri_init_cb(RAFT_INIT_SHUTDOWN_STATE);
    ...
    const struct raft_net_wr_supp *ws)
{
    ...
    if (ws->rnws_handle)
    {
        ...
        if (!is_user_cf)
        {
            LOG_MSG(LL_WARN, "KV_CRC_SKIP: key='%.*s' key_sz=%zu val_sz=%zu (not user CF)",
    ...
    LOG_MSG(LL_WARN, "KV_CRC_UPDATE: key='%.*s' key_sz=%zu val_sz=%zu old_running=0x%x new_running=0x%x",
            (int)MIN(ws->rnws_key_size, 64), ws->rnws_key ? ws->rnws_key : "(null)",
